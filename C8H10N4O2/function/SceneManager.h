...
public:
	...
	std::string tableMember = "";
	std::string Player_data[4];
	std::string tableMaster = "0";
	std::string gameMember = "0";
	std::string select_userID = "";
	std::string result_tip = "";
	bool actionflug[FUNC_COUNT] = { false };
	bool scenechange = false;
	Table tables[TABLE_NUM];
	void Table_initialize();
	void put_data(char* data,int i)
	{
		Player_data[i] = data;
	}
	//文字列infoをsignで分割してanserの配列に入れる
	void divideinfo(char* info, char** anser, char* sign)
	{

		char* result;
		result = strtok(info, sign);
		anser[0] = result;
		int i = 0;
		while (result != NULL)
		{
